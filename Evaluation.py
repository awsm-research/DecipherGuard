...
def main():
    ...
    if 'True' in args.baseline:
        ...
        for dataset_name, prompts in datasets.items():
            ...
            # Transform prompts to the required format
            transformed_prompts = transform_dataset_prompt(prompts)
            ...
            output_file = os.path.join(output_dir, f"Baseline_{dataset_name}_metrics.csv")
    ...
    for attack_name in selected_attacks:
        ...
        for dataset_name, prompts in datasets.items():
            ...
            output_dir = 'results'
            os.makedirs(output_dir, exist_ok=True)
            output_file = os.path.join(output_dir, f"{attack_name}_{dataset_name}_metrics.csv")
            metrics_df.to_csv(output_file, index=False)
