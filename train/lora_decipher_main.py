...
def tokenize_with_template_decipher(user_inputs, agent_labels):
    input_ids_list = []
    labels_list = []
    for user_input, agent_label in tqdm(zip(user_inputs, agent_labels), desc="Tokenizing"):
        ...
        if model_input is not False:
            ...
            conf = detector.compute_language_confidence_values(user_input)[0]
            if conf.language.name == "ZULU":
                model_input = translator.translate(user_input, dest="en", src="zu").text
            else:
                potential_probs = []
                potential_prompts = []
                for shift in range(0, 26, 1):
                    ...
                    if shift != 0:
                        encrypted = caesar_encrypt(user_input, shift)
...
if args.do_train:
    train_encodings = tokenize_with_template_decipher(train_texts, train_labels)
